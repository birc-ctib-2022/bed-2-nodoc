...
from bed import (read_bed_file, print_line, Table)


def sort_file(table: Table):
    ...
    for chrom, features in table.items():
        ...
        table[chrom] = sorted(features, key=lambda x: (x[1], x[2], x[3]))  # features should be sorted here
